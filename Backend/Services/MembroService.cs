...
public sealed class MembroService : IMembroService
{
    ...
    {
        var membro = await _repo.GetByIdAsync(membroId) ?? throw new KeyNotFoundException("Convite inexistente");
        if (membro.EstadoConvite != "Pendente") throw new InvalidOperationException("Convite já respondido");

        membro.EstadoConvite = aceitar ? "Aceite" : "Recusado";
        membro.DataEstado    = DateTime.UtcNow;
